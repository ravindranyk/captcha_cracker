...
def Train(train_fn, image_input, target_chars, count_imagestrained_sofar, eval_matrix,
  ...
  print("Training")
  print("image",image_input.shape,"chars",target_chars.shape)
  ...
  for image_input_batch, target_chars_batch in IterateMinibatches(
    ...
    print ("Entered here ",image_input_batch.shape,"target",target_chars_batch.shape)
    if use_mask_input:
    ...
    train_err += l_err
    train_batches += 1
    print("train error",train_err,"train batches",train_batches)
...
def Test(test_fn, image_input, target_chars, total_images_trained, train_flag, eval_matrix,
  ...
  for image_input_batch, target_chars_batch in IterateMinibatches(
    ...
    if True:#multi_chars:
      err, acc, seq_acc,pred,targ,corr = tuple(out)
    else:
      err, acc = tuple(out)
    test_err += err
    test_acc += acc
...
def Run(args, num_epochs=7, multi_chars=True, num_softmaxes=None):
  ...
  captcha_model = Model(
      ...
      multi_chars=True, num_softmaxes=num_softmaxes,
  ...
  for epoch_num in range(num_epochs):
    for i, training_file in enumerate(
      ...
      Train(captcha_model.GetTrainFn(), image_input, target_chars,
            ...
            use_mask_input=args.use_mask_input)
      print ("train for",epoch_num,"done")
      _SaveModelAndRemoveOldOnes(captcha_model, model_params_file_prefix)
  ...
  Test(captcha_model.GetTestFn(), test_image_input,
       test_target_chars,100,False,eval_matrix,multi_chars=multi_chars,
       use_mask_input=args.use_mask_input)'''
 
class CaptchaCracker(object):
  def __init__(self, model_params_file_prefix, includeCapital,
               graph_file_path= "model_graph.png",
...
if __name__ == '__main__':
  ...
  Run(arguments)
  '''c = CaptchaCracker("../params/_2017_03_26_13_56_55.npy.npz",True)
  predicted,prob = c.InferFromImagePath("/home/ravindra/Desktop/variable_dataset_500/10_0aa0z0.jpg")
  #predicted , prob = c.InferForImageArray(arr)
  print("predicted",predicted,"prob",prob)'''
